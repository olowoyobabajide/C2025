...
void server_thread(void *size)
{
	int server_fd = * (int*)size;
	struct sockaddr_in address;
	int addr_len = sizeof(address);
	char *messsage = "Hello from server";
	char mess[BUFFER_SIZE] = {0};
	...
	while(1)
	{
		if (recvfrom(server_fd, mess, BUFFER_SIZE, 0, (struct sockaddr*)&address, (socklen_t*)&add_len) < 0)
		{
			perror("No message gotten from the client\n");
			return(EXIT_FAILURE);
		}
	}
}
